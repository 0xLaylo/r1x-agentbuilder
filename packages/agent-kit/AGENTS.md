...
```ts
import { createAgentApp, paymentsFromEnv } from '@r1x/agent-kit';
import type { EntrypointDef, AgentMeta } from '@r1x/agent-kit/types';
import type { AP2Config } from '@r1x/agent-kit/types';
```

Subpath exports are available:

- `@r1x/agent-kit` — main API
- `@r1x/agent-kit/types` — public types
- `@r1x/agent-kit/utils` — helpers (e.g., `toJsonSchemaOrUndefined`, `paymentsFromEnv`)
...
> Environment variables (`FACILITATOR_URL`, `PAYMENTS_RECEIVABLE_ADDRESS`, `NETWORK`,
> `DEFAULT_PRICE`, `R1X_API_URL`, etc.) are still respected. Values supplied
...
`@r1x/agent-auth` and reuse its `AgentRuntime` helpers instead of the
...
Pull in `createIdentityRegistryClient` from `@r1x/agent-kit/erc8004` to read/write the registry with whichever viem/ethers client you already use:

```ts
import {
  createIdentityRegistryClient,
  signAgentDomainProof,
} from '@r1x/agent-kit/erc8004';
import type { TrustConfig } from '@r1x/agent-kit/types';
