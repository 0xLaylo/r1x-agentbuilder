...
patterns for `@r1x/agent-kit` and `@r1x/agent-kit-hono`:
...
`@r1x/agent-kit-hono` offers:

- Building an agent server with `createAgentApp` (from `@r1x/agent-kit-hono`), including streaming entrypoints.
...
- Ready to pair with `@r1x/agent-auth` when you need authenticated wallet flows.
...
```
FACILITATOR_URL              # x402 facilitator endpoint (defaults to https://facilitator.payai.network)
...
AGENT_REF         # Agent reference used with the r1x wallet API
API_BASE_URL      # Base URL for the r1x agent API (defaults to https://localhost:8787)
...
R1X_AGENT_BASE_URL        # Auth server base URL for challenge/exchange (falls back to API_BASE_URL)
R1X_AGENT_AGENT_REF       # Agent ref to authenticate (falls back to AGENT_REF)
R1X_AGENT_CREDENTIAL_ID   # Credential ID that will sign challenges
R1X_AGENT_REFRESH_TOKEN   # Optional refresh token to seed the runtime cache
R1X_AGENT_SCOPES          # Optional scopes (JSON array or comma-separated string)
...
When the AgentRuntime variables are present the script signs the r1x
...
- a mock r1x auth/API surface that issues short-lived tokens and serves `/v1/agents`
