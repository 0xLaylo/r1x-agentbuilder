...
async function setupAgentRuntime(options: {
  ...
  const baseUrl =
    process.env.R1X_AGENT_BASE_URL ??
    process.env.AGENT_AUTH_BASE_URL ??
    process.env.API_BASE_URL;
  const agentRef = process.env.R1X_AGENT_AGENT_REF ?? process.env.AGENT_REF;
  const credentialId =
    process.env.R1X_AGENT_CREDENTIAL_ID ?? process.env.CREDENTIAL_ID;
  const refreshToken =
    process.env.R1X_AGENT_REFRESH_TOKEN ?? process.env.REFRESH_TOKEN;
  const scopes =
    parseScopes(process.env.R1X_AGENT_SCOPES) ??
  ...
  if (!baseUrl || !agentRef || !credentialId) {
    console.warn(
      '[examples] AgentRuntime auth skipped; provide R1X_AGENT_BASE_URL, R1X_AGENT_AGENT_REF, and R1X_AGENT_CREDENTIAL_ID to enable challenge flow'
    );
    return null;
  }
    ...
    const { app, addEntrypoint } = createAgentApp(
      {
        ...
        version: '0.1.0',
        description:
          'Demonstrates every feature exposed by @r1x/agent-kit. A2A, x402, ERC-8004, and more.',
      },
      {
