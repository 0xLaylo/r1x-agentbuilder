...
`@r1x/agent-kit` is the core runtime for building AI agents with typed entrypoints, discovery endpoints, monetization hooks, and trust metadata. It provides the shared runtime logic used by adapter packages like `@r1x/agent-kit-hono` and `@r1x/agent-kit-tanstack`.
...
import type { EntrypointDef, AgentMeta } from '@r1x/agent-kit/types';
```

**TanStack Adapter:**

```ts
import { createTanStackRuntime } from '@r1x/agent-kit-tanstack';
import type { EntrypointDef, AgentMeta } from '@r1x/agent-kit/types';
```

Subpath exports (shared across adapters):

- `@r1x/agent-kit/types` — public TypeScript interfaces and helper types
- `@r1x/agent-kit/utils` — focused helpers (`toJsonSchemaOrUndefined`, `paymentsFromEnv`, etc.)
...
r1x Agent SDK supports payment receiving on multiple blockchain networks:
...
- Environment variables override defaults automatically: `FACILITATOR_URL`, `PAYMENTS_RECEIVABLE_ADDRESS`, `NETWORK`, `DEFAULT_PRICE`, `R1X_API_URL`/`VITE_API_URL`, `AGENT_WALLET_MAX_PAYMENT_BASE_UNITS`, and `AGENT_WALLET_MAX_PAYMENT_USDC`.
...
`@r1x/agent-auth` and reuse the generated SDK surface:

```ts
import { AgentRuntime } from '@r1x/agent-auth';
import { createRuntimePaymentContext } from '@r1x/agent-kit';
...
const { runtime } = await AgentRuntime.load({
  ...
  loader: {
    overrides: {
      baseUrl: process.env.R1X_API_URL,
...
Trust metadata is modelled by `TrustConfig`. For ERC-8004 identity management, use the dedicated `@r1x/agent-kit-identity` package:
...
See [`@r1x/agent-kit-identity` documentation](../agent-kit-identity/README.md) for complete examples and API reference.
