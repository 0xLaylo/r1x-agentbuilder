...
function printHelp(logger: RunLogger) {
  logger.log('Usage: bunx @r1x/create-agent-kit <app-name> [options]');
  logger.log('');
  logger.log('Options:');
  logger.log(
  ...
  logger.log('');
  logger.log('Examples:');
  logger.log('  bunx @r1x/create-agent-kit my-agent');
  logger.log(
    '  bunx @r1x/create-agent-kit my-agent --network=solana-devnet'
  );
  logger.log(
    '  bunx @r1x/create-agent-kit my-agent --template=identity --install'
  );
  logger.log('  bunx @r1x/create-agent-kit my-agent --wizard=no');
  logger.log('');
  logger.log('Non-interactive with template arguments:');
  logger.log(
    '  bunx @r1x/create-agent-kit my-agent --template=identity \\'
  );
...
function printBanner(logger: RunLogger) {
  R1X_BANNER.forEach(line => logger.log(line));
}
